...
SQLITE_SOURCE_BN = sqlite-src-$(SQLITE_TAR_VERSION)
SQLITE_SOURCE = $(SQLITE_SOURCE_BN).zip
SQLITE_SITE = https://www.sqlite.org/2022
SQLITE_LICENSE = Public domain
SQLITE_LICENSE_FILES = autoconf/tea/license.terms
SQLITE_CPE_ID_VENDOR = sqlite
SQLITE_INSTALL_STAGING = YES

define SQLITE_EXTRACT_CMDS
        $(UNZIP) -d $(@D) $(SQLITE_DL_DIR)/$(SQLITE_SOURCE)
	mv $(@D)/$(SQLITE_SOURCE_BN)/* $(@D)
endef
...
ifeq ($(BR2_PACKAGE_SQLITE_ENABLE_UPDATE_DELETE_LIMIT),y)
SQLITE_CONF_OPTS += --enable-update-limit
SQLITE_CFLAGS += -DSQLITE_ENABLE_UPDATE_DELETE_LIMIT
endif
